...
# Introduction
This is the report for the first interim analysis of the NOR-SOLIDARITY trial. The data are based on an export from the Viedoc electronic data capture at "`r export_date`" system time stamped "`r params$viedoc_export`". There were `r n_distinct(adsl)` included patients.
...
addm_tot <- adsl %>% select(-age_calc, -sq_admis, -sex) %>% 
  left_join(tddm, by ="subjectid") %>% filter(fas == "Yes")
addm_hcq <- adsl  %>% select(-age_calc, -sq_admis, -sex) %>% 
  left_join(tddm, by ="subjectid") %>% filter(fas_hcq == "Yes")
addm_rem <- adsl  %>% select(-age_calc, -sq_admis, -sex) %>% 
  left_join(tddm, by ="subjectid") %>% filter(fas_rem == "Yes")


dm_table_f(addm_tot, dm_table, "Demographics, all arms (FAS)", header_tot)
...
dm_table_f(addm_tot, missing_table, "Missing values, all arms (FAS)", header_tot)

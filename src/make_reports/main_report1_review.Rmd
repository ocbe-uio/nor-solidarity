...
vl <- vlbl %>% 
  ...
  mutate(Timepoint = factor(Timepoint, levels = c("diff_7", "diff_10")))
...
advl %>% filter(fas_hcq == "Yes") %>% 
  ...
  knitr::kable(caption = "Counts for HCQ", col.names = c("Treatment", "Period", "Number with non-missing values"))

advl %>% filter(fas_rem == "Yes") %>% 
  group_by(rantrt, epoch) %>% 
  summarise(n = n(), .groups = "drop_last" ) %>% 
  mutate(epoch = case_when(
    epoch == 0 ~ "Baseline",
    epoch == 4 ~ "Day 1 to 5",
  ...
  knitr::kable(caption = "Counts for remdesivir", col.names = c("Treatment", "Period", "Number of patients with non-missing values"))

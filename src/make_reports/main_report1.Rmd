...
RR_f <- function(diff){
  ...
  txt <- paste0("Estimated relative risk: ", round(exp(RR), digits = 2), " (95% CI ", 
...
survres <- tibble(
  ...
  mutate(fit = map2(formula, data, ~surv_fit(formula = .x, data = .y)),
         plot = map2(fit, data, ~ggsurvplot(fit = .x, data = .y)),
         diff = map2(formula, data, ~survdiff(formula = .x, data = .y)),
         chisq = map(diff, ~.x$chisq),
         pval = map(diff, ~round(1-pchisq(.x$chisq, length(.x$obs)-1), digits = 3)),
         RR = map(diff, ~RR_f(.x)))
...
# 
# 
# fit <- surv_fit(formula.list, data = adev)
# diff <- survdiff(formula.list, data = adev)
# ggsurvplot(fit, data = adev, risk.table = TRUE, pval = TRUE, fun = "event") 
# 
# fit <- surv_fit(formula.list, data = adev %>% filter(fas_hcq == "Yes"))
# ggsurvplot(fit, data = adev %>% filter(fas_hcq == "Yes"), risk.table = TRUE, pval = TRUE, fun = "event") 
# 
# fit <- surv_fit(formula.list, data = adev %>% filter(fas_rem == "Yes"))
# ggsurvplot(fit, data = adev %>% filter(fas_rem == "Yes"), risk.table = TRUE, pval = TRUE, fun = "event") 

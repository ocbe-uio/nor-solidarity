...
rdvl_sg <- readr::read_rds("results/rds/rdvl_sg.rds") %>% 
  mutate(filename = paste0(str_to_upper(str_remove(population, "fas_")), " Viral Load ",  sg_label))
...
rdvl_sg %>%
  filter(population == "fas_hcq") %>% 
  mutate(walk(plot2, ~plot(.x + scale_color_manual(values = trt_colors)))) %>% 
  invisible()
...
adab %>% 
  summarise(total = n(), rbd = sum(abseroc=="RBD ≥ 5"), capsid = sum(abcapsidd == "Capsid ≥ 10")) %>% 
  knitr::kable(caption = "Number of seroconverted out of total")
...
rdlb <- read_rds("results/rds/rdlb.rds") %>% 
  mutate(filename = paste0(str_to_upper(str_remove(population, "fas_")), "_", var ))
...
efflbres %>%
  mutate(marginsplot = map2(marginsplot, title, ~ .x + ggtitle(.y) + scale_color_manual(values = trt_colors)),
         walk(marginsplot, print),
         walk2(marginsplot, filename, save_plot_pdf))  %>%
  invisible()
